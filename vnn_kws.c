/****************************************************************************
*   KWS Neural Network implementation
*   Based on vnn_voiceprintuint8.c structure
****************************************************************************/
/*-------------------------------------------
                   Includes
 -------------------------------------------*/
...
/* NEW_CONST_TENSOR - Not used since NBG loads weights internally
    ...
    )
{
    ...
    (void)(_infinity);
    (void)(enable_from_handle);  // Mark as intentionally unused
    ctx = NULL;
    graph = NULL;
    status = VSI_FAILURE;
...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    if( !inference_with_nbg )
    {
        /*-----------------------------------------
          KWS NBG Node
          input     - [40, 49, 1] (MFE features)
          output    - [2, 1] (unknown, wakeword)
        -----------------------------------------*/
...
/*-----------------------------------------
 ...
 -----------------------------------------*/
    ...
    /* Output tensor: [2, 1] - [unknown_prob, wakeword_prob] */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 2;
    attr.size[1] = 1;
    attr.dim_num = 2;
    attr.dtype.scale = 0.00390625;  // 1/256 for int8 quantization
    attr.dtype.zero_point = -128;
    attr.dtype.qnt_type = VSI_NN_QNT_TYPE_AFFINE_ASYMMETRIC;
    NEW_NORM_TENSOR(norm_tensor[0], attr, VSI_NN_TYPE_INT8);

    /* Input tensor: [40, 49, 1] - MFE features */
    memset( &attr, 0, sizeof( attr ) );
    attr.size[0] = 40;
    attr.size[1] = 49;
    attr.size[2] = 1;
    attr.dim_num = 3;
    attr.dtype.scale = 0.0862745;  // Typical MFE scale
...
error:
    ...
    vnn_ReleaseKWS( graph, release_ctx );
